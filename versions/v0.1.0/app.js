...
function cardImage(item) {
  ...
  if (item.library === "built-in" && localCards.has(item.word)) return `./assets/cards/${item.word}.svg`;
...
function addXp(amount) {
  ...
  if (after > before && !state.pendingLevelUp) {
    state.pendingLevelUp = { from: before, to: after, score: 0 };
  }
}
...
function render() {
  ...
  if (screen === "minigame") app.innerHTML = renderMiniGame();
  ...
  return appShell(`
    <section class="dashboard">
      <div class="panel game-panel">
        ...
        ${state.pendingLevelUp ? `<div class="quest-alert"><strong>${t("levelUp")} Lv.${state.pendingLevelUp.from} -> Lv.${state.pendingLevelUp.to}</strong><button class="btn" data-action="minigame">${t("playMiniGame")}</button></div>` : ""}
...
function renderMiniGame() {
  if (!miniGame) {
    miniGame = { score: 0, time: 15, activeRune: 0, runes: ["A", "B", "C", "D"], timer: null };
  }
  if (!miniGame.timer) {
    miniGame.timer = setInterval(() => {
      miniGame.time = Math.max(0, miniGame.time - 1);
      updateMiniGameDom();
      if (miniGame.time <= 0) {
        clearInterval(miniGame.timer);
        miniGame.timer = null;
        updateMiniGameDom(true);
      }
    }, 1000);
  }
  const done = miniGame.time <= 0;
  return appShell(`
    <section class="panel retro-game">
      <div>
        <div class="eyebrow">${t("levelUp")}</div>
        <h2 class="page-title">${t("miniTitle")}</h2>
        <p class="hero-copy">${t("miniCopy")}</p>
        <div class="stats">
          <div class="stat"><strong id="miniScore">${miniGame.score}</strong><span>${t("miniScore")}</span></div>
          <div class="stat"><strong id="miniTime">${miniGame.time}</strong><span>${t("miniTime")}</span></div>
          <div class="stat"><strong id="miniReward">+${20 + miniGame.score * 4}</strong><span>${t("xp")}</span></div>
        </div>
      </div>
      <div class="rune-grid">
        ${miniGame.runes.map((rune, index) => `<button class="rune ${index === miniGame.activeRune && !done ? "active" : ""}" data-rune="${index}" ${done ? "disabled" : ""}>${rune}</button>`).join("")}
      </div>
      <div class="actions">
        <button class="btn" id="claimMiniGame" data-action="claim-minigame" ${done ? "" : "disabled"}>${t("claimReward")}</button>
        <button class="btn secondary" data-action="dashboard">${t("back")}</button>
      </div>
    </section>
  `);
    ...
    <section class="panel import-panel">
      ...
      <div class="form-grid">
        ...
        <div class="field">
          ...
          <textarea class="textarea" id="wordImportText" rows="10" placeholder="${t("importPlaceholder")}"></textarea>
        </div>
      </div>
      <div class="actions">
...
function bindActions() {
  ...
  document.querySelectorAll("[data-rune]").forEach((button) => {
    button.addEventListener("click", () => hitRune(Number(button.dataset.rune)));
  });
  document.querySelectorAll("[data-action='claim-minigame']").forEach((button) => {
    button.addEventListener("click", () => claimMiniGameReward());
  });
...
function hitRune(index) {
  if (!miniGame || miniGame.time <= 0) return;
  if (index === miniGame.activeRune) {
    miniGame.score += 1;
    miniGame.activeRune = Math.floor(Math.random() * miniGame.runes.length);
  } else {
...
function updateMiniGameDom(done = false) {
  if (!miniGame) return;
  const score = document.querySelector("#miniScore");
  const time = document.querySelector("#miniTime");
  const reward = document.querySelector("#miniReward");
  const claim = document.querySelector("#claimMiniGame");
  if (score) score.textContent = miniGame.score;
  if (time) time.textContent = miniGame.time;
  if (reward) reward.textContent = `+${20 + miniGame.score * 4}`;
  document.querySelectorAll("[data-rune]").forEach((button) => {
    const active = Number(button.dataset.rune) === miniGame.activeRune && miniGame.time > 0;
    button.classList.toggle("active", active);
    button.disabled = miniGame.time <= 0;
  });
  if (claim) claim.disabled = !(done || miniGame.time <= 0);
}

function claimMiniGameReward() {
  if (!miniGame || miniGame.time > 0) return;
  addXp(20 + miniGame.score * 4);
...
function saveImportedLibrary() {
  ...
  const imported = parseImportedWords(raw);
  if (!imported.length) {
    toast = t("importCopy");
    render();
    return;
...
function parseImportedWords(raw) {
  return raw
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean)
    .map((line, index) => {
      ...
      const meaning = parts[1] || parts[0];
      const example = parts[2] || `I want to remember ${wordText}.`;
...
function getSimilarWords(item) {
  const source = activeWords().length ? activeWords() : allWords();
  ...
  const fallback = source.filter((candidate) => candidate.word !== item.word);
  return uniqueWordObjects([...shuffle(nearLevel), ...shuffle(samePos), ...shuffle(fallback)]).slice(0, 3);
}
