...
function renderAchievements() {
  ...
  const rows = [
    [t("achTotalCorrect"), a.totalCorrect],
    [t("achBestCombo"), a.bestCombo],
    [t("achLoginStreak"), state.streakDays || 0],
    [t("achTotalLogin"), a.totalLoginDays],
    [t("achFirstLevel"), a.firstLevelUp ? "✓" : "0"],
    [t("achMonster"), a.monstersDefeated],
  ];
  return appShell(`
    <section class="panel">
      ...
      <div class="grid">
        ${rows.map(([label, value]) => `<article class="card achievement"><strong>${value}</strong><span>${label}</span></article>`).join("")}
      </div>
...
function getSimilarWords(item) {
  ...
  return uniqueWordObjects([...shuffle(nearLevel), ...lookAlikes.slice(0, 6), ...shuffle(samePos), ...shuffle(fallback)]).slice(0, 3);
}
