...
- Added unique SVG card artwork for all 40 built-in words.
- Built-in word cards now always use their own dedicated image file.
- Quiz distractors now come from the full word pool, not only recently studied words.
- Distractors are constrained to the same part of speech first, with visually similar words prioritized.
- Rebuilt achievements as badge cards with chuuni-style names, distinct badge art, clear unlock conditions, and progress bars.
